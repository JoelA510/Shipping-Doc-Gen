...
FORWARDER_DIR = os.path.join(BASE_DIR, "Forwarder")
ALT_FORWARDER_DIR = os.path.join(BASE_DIR, "AlternateForwarder")
...
def process_documents():
    ...
    # 2. Process Carrier Files (Forwarder)
    # We'll walk through the directory
    print("Processing Forwarder files (this may take a while)...")
    for root, dirs, files in os.walk(FORWARDER_DIR):
        for file in files:
            if file.lower().endswith(".pdf"):
                ...
                for shipment_id in jde_map.keys():
                    if shipment_id in text:
                        ...
                        # Copy and rename
                        group_path = os.path.join(OUTPUT_DIR, shipment_id)
                        dst = os.path.join(group_path, f"FWD_{shipment_id}.pdf") # Or append if multiple?
